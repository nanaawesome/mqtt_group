...
void loop() {
  ...
  if (now - lastMsg > 200) {
    lastMsg = now;
    //Read ESP32 CPU Temperature
    float temp = temperatureRead();
    // Convert the value to a char array
    char distString[8];
    dtostrf(temp, 1, 2, distString);
    client.publish("esp32/temperature", distString);
  }
...
float temperatureRead() {
  uint8_t temp = 0;
  float temperature = 0;
  temp = temperatureReadByte();
  temperature = (temp - 32) / 1.8;
  return temperature;
}

uint8_t temperatureReadByte() {
  uint8_t temperature = 0;
  temperature = READ_PERI_REG(SENS_SAR_TEMP_ADC1_REG) >> 16;
  return temperature;
}
